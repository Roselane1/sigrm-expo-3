...
import { ChevronLeft, ChevronDown, ChevronUp, CheckCircle2, XCircle, Info, Printer, Trash2 } from "lucide-react-native";
    ...
    <ScrollView contentContainerStyle={{ paddingBottom: 60 }}>
      ...
      <View style={{ gap: 10 }}>
        {req.items.map((it) => {
          ...
          const expanded = expandedIds.has(it._id);
          const ExpandIcon = expanded ? ChevronUp : ChevronDown;
 
          return (
            <Card key={it._id} style={{ opacity: cancelado ? 0.6 : 1, padding: 0, overflow: "hidden" }}>
              <Pressable onPress={() => toggleExpand(it._id)} style={styles.itemHeaderRow}>
                <View style={{ flex: 1 }}>
                ...
                <ExpandIcon size={18} color={COLORS.muted} />
              </Pressable>
 
              {expanded && (
                <View style={styles.itemBody}>
                  {isTecnico && !locked && !cancelado && (
                    <Pressable onPress={() => cancelItem(it._id)} style={styles.cancelItemBtn}>
                      <Trash2 size={14} color={COLORS.rust} />
                      <Text style={styles.cancelItemText}>Cancelar item</Text>
                    </Pressable>
                  )}
 
                  {canEditTecnico && (
                    <View style={{ marginBottom: 10 }}>
                      <Text style={styles.itemFieldLabel}>Descrição do Material</Text>
                      <TextField value={it.descricaoMaterial} onChangeText={(v) => handleFieldChange(it._id, "descricaoMaterial", v)} />
                    </View>
                  )}
 
                  <Text style={[styles.itemCode, { marginBottom: 10 }]}>Nº de NT: {it.obs || "—"}</Text>
                  ...
                  <View style={[styles.itemFieldsRow, { marginTop: 10 }]}>
                    <View style={styles.itemField}>
...
const styles = StyleSheet.create({
  ...
  itemHeaderRow: { flexDirection: "row", alignItems: "center", gap: 10, padding: 14 },
  itemBody: { paddingHorizontal: 14, paddingBottom: 14, borderTopWidth: 1, borderTopColor: COLORS.line, paddingTop: 12 },
  itemName: { fontFamily: FONTS.bodyMedium, fontSize: 13.5, color: COLORS.ink, marginTop: 2 },
  itemCode: { fontFamily: FONTS.mono, fontSize: 11, color: COLORS.muted },
  cancelItemBtn: { flexDirection: "row", alignItems: "center", gap: 6, alignSelf: "flex-start", marginBottom: 12 },
  cancelItemText: { color: COLORS.rust, fontSize: 12, fontFamily: FONTS.bodySemiBold },
  itemFieldsRow: { flexDirection: "row", gap: 20, flexWrap: "wrap" },
  itemField: { gap: 3 },
